...
<p align="center">
<strong>Latest Changes (v1.28):</strong> **P1 SPECIFICATION GAPS RESOLVED:** Added Section 5.6 Chain IDs and RPC Compatibility (precise chainId conventions for Primary Network 1-99, States 1000-999999, Cities 1000000+, EIP-155 replay protection, Federal Chain discovery registry with signed metadata, RPC behavior matrix, wallet integration patterns). Added Section 10.1.1 Checkpoint Verification Algorithm (canonical validator set tracking via Federal Chain registry, BLS aggregate signature verification, light client support, mid-epoch validator set change handling). Added Section 4.4.1 City Emergency Exit and Fraud Proofs (Merkle proof-based balance recovery, Federal Chain adjudication, griefing prevention). Updated Section 4.5.3 Code Vault Canonical Encoding (TLV format replacing JSON for consensus-critical structures). Added version markers: (v1) mainnet-implemented, (vNext) optional/research for DAS and ZK-EVM, (future) speculative. Total additions: ~900 lines. Addresses auditor feedback on consensus-critical encoding, developer UX, and City safety guarantees. Previous (v1.27): Section numbering consistency fixes.
</p>
